...
static bool deskquakeHadQuake = false;
...
enum DeskQuakeState {
    ...
    DQ_POWER_WAIT_HIGH,
    DQ_BEGIN_TRY,
    DQ_BEGIN_FAILED_WAIT,
    DQ_WAIT_STABLE_BEFORE_INIT,
    DQ_INITIALIZE,
    DQ_WAIT_READY,
    DQ_RUN
};

static DeskQuakeState deskquakeState = DQ_OFF;
static uint32_t deskquakeStateSinceMs = 0;
static uint32_t deskquakeLastReadyCheckMs = 0;

static const uint32_t deskquakePowerLowMs = 1000;
static const uint32_t deskquakePowerHighSettleMs = 1000;
...
static void deskquakeFormatElapsed(char *buf, size_t len, uint32_t elapsedMs)
{
    ...
    if (hours == 0 && minutes == 0)
        snprintf(buf, len, "0h00m%02us", seconds);
    else
        snprintf(buf, len, "%uh%02um", hours, minutes);
}

static bool deskquakePingD7S(TwoWire &bus)
{
    bus.beginTransmission(0x55);
    return bus.endTransmission() == 0;
}

static void deskquakeSendMeshAlert(float si, float pga)
{
    if (!service || !router) {
        LOG_WARN("DeskQuake alert not sent: mesh service unavailable");
        return;
    }

    if (deskquakeAlertChannelIndex >= channels.getNumChannels()) {
        LOG_WARN("DeskQuake alert not sent: channel %u unavailable", deskquakeAlertChannelIndex);
        return;
    }
    ...
    char message[128];
    snprintf(message, sizeof(message), "DeskQuake alert on %s: earthquake detected (SI=%.3f PGA=%.3f)",
             deskquakeAlertChannelLabel, si, pga);
...
static void probeD7SBus(TwoWire &bus, const char *name)
{
    ...
    LOG_INFO("D7S raw probe %s err=%u", name, err);
    ...
    if (n0 == 1) {
        uint8_t v0 = bus.read();
        LOG_INFO("D7S reg0x00 %s = 0x%02X", name, v0);
    }
...
static void deskquakeResetStats(bool resetSensorEvents)
{
    ...
    deskquakeLastQuakeEndMs = 0;
    deskquakeHadQuake = false;
    deskquakeQuakeCount = 0;

    if (resetSensorEvents && deskquakeReady) {
        d7s.resetEvents();
    }

    LOG_INFO("DeskQuake stats reset");
}

static void deskquakeLogStatus(uint32_t now, bool ready, bool eq, float si, float pga)
{
    char elapsed[24];
    if (deskquakeHadQuake) {
        deskquakeFormatElapsed(elapsed, sizeof(elapsed), now - deskquakeLastQuakeEndMs);
    } else {
...
bool handleDeskQuakeConsoleCommand(const char *command)
{
    ...
    if (strcmp(command, "dqdfu") == 0) {
        LOG_INFO("DeskQuake entering DFU mode");
        enterDfuMode();
        return true;
    }

    if (strcmp(command, "dqreset") == 0) {
        deskquakeResetStats(true);
        return true;
    }
...
static void deskquake_startup()
{
    deskquakeReady = false;
    deskquakeInitializing = false;
    deskquakeRuntimeReadyLogged = false;
    deskquakeBeginLogged = false;
    deskquakeLastReadMs = 0;
    deskquakeLastReadyCheckMs = 0;
    deskquakeQuakeAlertSent = false;
    deskquakeWasEq = false;
...
static void deskquake_service()
{
    uint32_t now = millis();

    switch (deskquakeState) {
    case DQ_OFF:
        deskquake_startup();
        break;
    ...
    case DQ_POWER_LOW:
        ...
        LOG_INFO("DeskQuake power LOW");
        deskquakeSetState(DQ_POWER_WAIT_LOW);
        break;

    case DQ_POWER_WAIT_LOW:
        if (now - deskquakeStateSinceMs >= deskquakePowerLowMs) {
            deskquakeSetState(DQ_POWER_HIGH);
        }
        break;

    case DQ_POWER_HIGH:
        digitalWrite(WB_IO2, HIGH);
        LOG_INFO("DeskQuake power HIGH");
        deskquakeSetState(DQ_POWER_WAIT_HIGH);
        break;

    case DQ_POWER_WAIT_HIGH:
    ...
    case DQ_BEGIN_FAILED_WAIT:
        if (now - deskquakeStateSinceMs >= deskquakeRetryAfterFailMs) {
            deskquakeSetState(DQ_POWER_LOW);
        }
        break;

    case DQ_WAIT_STABLE_BEFORE_INIT:
        if (now - deskquakeStateSinceMs >= deskquakeStableBeforeInitMs) {
            deskquakeSetState(DQ_INITIALIZE);
        }
        break;

    case DQ_INITIALIZE:
        LOG_INFO("DeskQuake setAxis(SWITCH_AT_INSTALLATION)");
        d7s.setAxis(SWITCH_AT_INSTALLATION);
        delay(2000);
        LOG_INFO("DeskQuake initialize()");
        d7s.initialize();
        deskquakeInitializing = true;
        deskquakeLastReadyCheckMs = 0;
        deskquakeSetState(DQ_WAIT_READY);
        break;

    case DQ_WAIT_READY:
        if (now - deskquakeLastReadyCheckMs < deskquakeReadyPollMs) {
            break;
        }
        deskquakeLastReadyCheckMs = now;
    ...
    case DQ_RUN:
        if (now - deskquakeLastReadMs < deskquakeReadIntervalMs) {
            break;
        }
        deskquakeLastReadMs = now;

        {
            bool ready = d7s.isReady();
            bool eq = d7s.isEarthquakeOccuring();
            ...
            if (eq && pga > deskquakePeakPGA) {
                deskquakePeakPGA = pga;
            }

            if (!eq && deskquakeWasEq) {
                float finalPga = d7s.getLastestPGA(0);
            ...
            if (!deskquakeRuntimeReadyLogged && ready) {
                LOG_INFO("DeskQuake runtime ready");
                deskquakeRuntimeReadyLogged = true;
            }

            deskquakeLogStatus(now, ready, eq, si, pga);
        }
        break;
    }
}
